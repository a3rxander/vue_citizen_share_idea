          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="item in BusquedaPropuesta" :key="item">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <div @click="$router.push({name: 'participation_Propuestas_show' , params: {id:item.id}})" class="text-indigo-600 hover:text-indigo-900">DIV</div>
              </td>
            </tr>
...
export default { 
   computed:{
       ...mapGetters('participation_storevuex',['getPropuestas']),
       BusquedaPropuesta(){
           return this.getPropuestas( this.s_buscar)
       },
       
   ...mapState('participation_storevuex',['isLoading'])
   },
   data(){
       return {
           s_buscar:''
 ...
 methods: {
   ...mapActions('participation_storevuex',['loadPropuestas'])
 },
 created(){
   this.loadPropuestas()
 },
