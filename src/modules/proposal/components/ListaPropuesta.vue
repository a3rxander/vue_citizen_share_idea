            ...
            <tr v-for="item in BusquedaPropuesta" :key="item">
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">
                  Active  {{item.v_status}}
                </span>
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <button class="bg-transparent hover:bg-blue-500 text-blue-700 font-semibold hover:text-white py-2 px-4 border border-blue-500 hover:border-transparent rounded" @click="$router.push({name: 'proposal_propuesta_show' , params: {id:item.id}})">Revisar</button>
              </td>
            </tr>
...
export default { 
   computed:{
       ...mapGetters('proposal_storevuex',['getPropuestas']),
       BusquedaPropuesta(){
           return this.getPropuestas( this.s_buscar)
       },
       
   ...mapState('proposal_storevuex',['isLoading'])
   },
   data(){
       return {
           s_buscar:''
 ...
 methods: {
   ...mapActions('proposal_storevuex',['loadPropuestas'])
 },
 created(){
   this.loadPropuestas()
 },
